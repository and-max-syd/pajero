...
The repair, stage by stage, front to back. Work one stage at a time. Do not rush. Each stage has a goal and a clear finish line. When a stage is done, rest, then start the next.

This is a living document. We change it as we learn.

## Progress so far

* Broke off the smashed plastic grille. This gave access to the bonnet latch.
* Unbolted the bent latch mechanism. Detached the release cable and the sensor wires.
* The bonnet is open. The engine bay is visible.
* A first look at the engine bay confirms the theory. Damage looks limited to the radiator, the fans, and the mounting brackets.

Stage 1 is done. The bonnet was opened while the car sat on the road. The car is still parked about ten minutes' drive from Ben. Next up: get the car home on a flatbed (Stage 2), then strip the front (Stage 3).
...
## Stage 1 — Open the bonnet — DONE

Goal: get the bonnet open so we can see the damage.
...
## Stage 5 — Source the parts

Goal: gather every part before major refitting.
